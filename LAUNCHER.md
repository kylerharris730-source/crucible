# Cinderlift launcher and releases

The public entry point is `cinderlift-launcher.exe`. A player downloads that file
once from a GitHub Release and can keep using it for later releases.

On startup the launcher downloads `cinderlift-manifest.txt` from the repository's
latest full release. It installs the game under `%LOCALAPPDATA%\Cinderlift`, checks
...
On first use it also copies any adjacent `cinderlift*.sav` files and
`cinderlift.cfg` into the install folder without overwriting data already there,
...
downloaded as `cinderlift-launcher.next.exe`; on the next open that verified copy
...
- `cinderlift-launcher.exe`
- `cinderlift.exe`
- `cinderlift-manifest.txt`
...
- `build\cinderlift.exe`
- `build\cinderlift-launcher.exe`
...
launcher detects an adjacent `cinderlift.exe`, imports it into the normal install
