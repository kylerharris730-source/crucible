...
/* Shared by every consumable heal. FIFTEEN seconds, halved from thirty after
   play: thirty was set against Terraria's full minute and read as reasonable
   next to it, but this game's fights are shorter than Terraria's and a lockout
   that outlasts the fight is not a cost inside the fight -- it is a wait
   afterwards. At fifteen a long exchange gets a second drink. */
static const int HEAL_COOLDOWN_FRAMES = 15 * 60;
