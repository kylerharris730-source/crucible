/* ============================================================================
   font8x8.h -- printable ASCII, 32..126, as 8 rows of 8 bits.

   Glyphs occupy bits 6..2 (five columns) and rows 0..6, leaving row 7 for the
   descenders of g j p q y. Bit 7 is the most significant, so the leftmost
   pixel of a row is 0x80 and the five glyph columns run 0x40 down to 0x04.

   Five columns rather than eight is deliberate. The Windows build measures
   text in Consolas, whose advance at the default 14px size is a shade under
   8px, and every panel in the game is laid out against that number. Drawing a
   glyph NARROWER than the advance and centring it in the cell keeps the
   advance -- and therefore every column, every centred label and every
   wrapped tooltip -- landing where the Windows layout puts it. A wider glyph
   would look denser and push all of that sideways, which is the one thing a
   substitute font must not do.
   ========================================================================== */
#pragma once
#include <stdint.h>

static const int FONT_FIRST = 32;
static const int FONT_LAST  = 126;
static const int FONT_ROWS  = 8;
static const int FONT_COLS  = 5;   /* glyph columns actually used */

static const uint8_t FONT8X8[(FONT_LAST - FONT_FIRST + 1) * FONT_ROWS] = {
    ...
    0x30,0x48,0x50,0x20,0x54,0x48,0x34,0x00, /* 38 & */
    ...
    0x7C,0x08,0x10,0x18,0x04,0x44,0x38,0x00, /* 51 3 */
    ...
    0x00,0x00,0x58,0x24,0x20,0x20,0x20,0x00, /* 114 r */
